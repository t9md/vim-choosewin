...
function! s:overlay.fill_space(lines, width) "{{{1
  let space = repeat(' ', a:width)
  for line in a:lines
    call setline(line,
          \ substitute(getline(line), "\t", repeat(" ", &tabstop), 'g') . space
          \ )
  endfor
endfunction
...
function! s:overlay.setup_winvar() "{{{1
  for winnr in self.wins
    ...
    let wh     = winheight(0)
    let ww     = winwidth(0)
    let line_s = w0 + 1 + (wh - s:font_height)/2
    let line_e = line_s + s:font_height
    let col    = (ww - s:font_width)/2
    let w:choosewin = {
          \ 'w0': w0,
          \ 'w$': w_,
          \ 'wh': wh,
...
function! s:overlay.setup(wins) "{{{1
  ...
  for bufnr in self.bufs
    call setbufvar(bufnr, 'choosewin',
          \ { 'rendering_area': [], 'options': {} } )
  endfor
endfunction

function! s:overlay.prepare_buffer() "{{{1
  for bufnr in self.bufs
    call s:undobreak()
    " noautocmd silent execute bufnr 'buffer'
    ...
    silent undojoin
    call self.fill_space(s:uniq(b:choosewin.rendering_area), s:render_width)
  endfor
  noautocmd execute self.winnr_org 'wincmd w'
endfunction
...
function! s:overlay.hl_label(is_current) "{{{1
  return matchadd(
        ...
        \ s:intrpl(self.next_font().pattern, s:vars(w:choosewin.pos)),
